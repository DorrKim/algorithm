const [N, ...bundleSizes] = require('fs').readFileSync('dev/stdin').toString().trim().split('\n').map(Number);
...
function solution(N, bundleSizes) {
  ...
  while (minHeap.size() > 1) {
    let poppedBundleSum = 0;

    poppedBundleSum += minHeap.pop();
    poppedBundleSum += minHeap.pop();

    result += poppedBundleSum;
    minHeap.add(poppedBundleSum);
  }

  console.log(result);
}

solution(N, bundleSizes);
