...
function solution(N, M, START, END, BUS_ROUTES) {
  ...
  const minRoutes = new Array(N + 1).fill(null);
  ...
  minHeap.add([dist[START], START, [START]]);

  while (minHeap.heap.length) {
    const [minDist, v1, visitCities] = minHeap.pop();
    if (visit[v1]) continue;
    visit[v1] = true;
    minRoutes[v1] = visitCities;

    adjacentList.get(v1).forEach((cost, v2) => {
      if (visit[v2] || minDist + cost >= dist[v2]) return;
      const newMinCost = minDist + cost;
      dist[v2] = newMinCost;
      minHeap.add([newMinCost, v2, [...visitCities, v2]]);
    });
  }

  console.log(dist[END]);
  console.log(minRoutes[END].length);
  console.log(minRoutes[END].join(' '));
}
