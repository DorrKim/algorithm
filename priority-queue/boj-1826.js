...
function solution(oilStations, initialState) {
  const [end, initialOil] = initialState;
  const priorityQueue = new MaxHeap();
  const oilAmounts = new Array(end + 1).fill(0);

  oilStations.forEach(([pos, oilAmount]) => {
    oilAmounts[pos] = oilAmount;
  });
  ...
  while (currPostion + currOil < end) {
    currPostion++;
    currOil--;

    if (oilAmounts[currPostion]) {
      priorityQueue.add(oilAmounts[currPostion]);
    }

    if (currOil === 0) {
      stop++;
      if (priorityQueue.size()) {
        currOil += priorityQueue.pop();
      } else {
        stop = -1;
        break;
      }
