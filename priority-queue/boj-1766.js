...
const solveOrderInfos = input.slice(1).map((info) => info.split(' ').map(Number));
...
function solution(N, M, solveOrderInfos) {
  const solveBefore = new Array(N + 1).fill(null);
  solveOrderInfos.forEach(([before, after]) => {
    solveBefore[after] = before;
  });
}

solution(N, M, solveOrderInfos);
